...
export function useAccumulatorTrading(wsUrl?: string, onAuthWSFailed?: () => void): UseAccumulatorTradingReturn {
  const {
    ws,
    isConnected,
    isLoading,
    ...
    sellError,
    clearSellError,
  } = useBaseTrading({ wsUrl, onAuthWSFailed, contractTypes: CONTRACT_TYPES });
